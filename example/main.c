...
int main(void)
{
    ...
    xf_sys_init(1000 * 1000 * 1000, _port_xf_sys_get_tick);

    xf_ms_t ms_before = xf_sys_get_ms();
    xf_delay_ms(500);
    xf_ms_t ms_after = xf_sys_get_ms();

    xf_us_t us_before = xf_sys_get_us();
    xf_delay_us(10);
    xf_us_t us_after = xf_sys_get_us();
